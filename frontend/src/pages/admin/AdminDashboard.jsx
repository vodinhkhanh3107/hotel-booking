      ...
      <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
        <Col xs={24} sm={12} lg={6}>
          <Card variant={false} hoverable style={{ borderRadius: '12px', borderLeft: '4px solid #52c41a' }}>
            <Statistic
              title={<Space>Doanh thu tháng này<Tooltip title="Tổng tiền sau khi trừ chiết khấu"><InfoCircleOutlined /></Tooltip></Space>}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={15}>
          <Card title="Biểu đồ tăng trưởng doanh thu" variant={false} style={{ borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            <Table
              ...
              size="middle"
              columns={[
                { title: 'Kỳ báo cáo', dataIndex: 'month', key: 'month', render: (text) => <Text strong>{text}</Text> },
                { 
                ...
                { title: 'Số lượng đơn', dataIndex: 'total_order', key: 'total_order', align: 'center' },
                {
                  title: 'So với tháng trước',
                  // dataIndex: 'revenue_difference',
                  key: 'revenue_difference',
                  render: (value) => (
                    value.revenue_difference !== 0 && <Tag icon={value.revenue_difference > 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />} color={value.revenue_difference > 0 ? 'green' : 'red'}>
                      {value.revenue_difference > 0 ? 'TĂNG' : 'GIẢM '}  {value.revenue_growth_percent}%
                    </Tag>
                  )
                }
              ]}
        ...
        <Col xs={24} lg={9}>
          <Card title="Khách sạn doanh thu cao nhất theo từng năm" variant={false} style={{ borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            {top_hotels && top_hotels.map((item, index) => (
              <div key={index} style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <Text strong style={{maxWidth: '180px'}} ellipsis>{item?.hotel_name}</Text>
                  <Text type="secondary" style={{fontSize: '13px'}}>{format_currency(item.total_revenue)}</Text>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2px' }}>
                  ...
                  <Text type="secondary" style={{ fontSize: '11px' }}>{item.total_order} đơn thành công</Text>
                </div>
              </div>
            ))}
          </Card>
